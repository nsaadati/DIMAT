...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, channel=3, w=1, num_classes=10, text_head=False):
        super(ResNet, self).__init__()
        self.in_planes = int(w*16)

        self.conv1 = nn.Conv2d(channel, int(w*16), kernel_size=3, stride=1, padding=1, bias=False)
...
def resnet20(channel=3, w=8, num_classes=10, text_head=False):
    return ResNet(BasicBlock, [3, 3, 3], channel, w=w, num_classes=num_classes, text_head=text_head)