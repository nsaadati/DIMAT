...
def vgg16_permutation_spec() -> PermutationSpec:
  ...
  return permutation_spec_from_axes_to_perm({
      ...
      **dense("classifier", "P_Conv_40", "P_Dense_0", False),
      # dimension for 32x32 images
      # **dense("classifier", (512,), "P_Dense_0", False),
      # dimension for 64x64 images
      # **dense("classifier", (2048,), "P_Dense_0", False),
})
...
def get_permuted_param(ps: PermutationSpec, perm, k: str, params, except_axis=None):
    ...
    for axis, p in enumerate(ps.axes_to_perm[k]):
        ...
        if p is not None:
            perm_tensor = perm[p].to('cuda').long()
...
def get_permuted_param(ps, perm, k, params, except_axis=None):
    """Get parameter `k` from `params`, with the permutations applied."""
    w = params[k].to('cuda')
    for axis, p in enumerate(ps.axes_to_perm[k]):
        # Skip the axis we're trying to permute.
...
def find_permutation(ps, params_a, params_b, max_iter=100, init_perm=None):
    """Find a permutation of `params_b` to make them match `params_a`."""
    perm_sizes = {p: params_a[axes[0][0]].shape[axes[0][1]] for p, axes in ps.perm_to_axes.items()}

    perm = {p: torch.arange(n).to('cuda') for p, n in perm_sizes.items()} if init_perm is None else init_perm
    ...
    for iteration in range(max_iter):
        ...
        for p_ix in torch.randperm(len(perm_names)):
            ...
            for wk, axis in ps.perm_to_axes[p]:
                w_a = params_a[wk].to('cuda')
                w_b = get_permuted_param(ps, perm, wk, params_b, except_axis=axis).to('cuda')
            ...
            oldL = torch.einsum('ij,ij->i', A, torch.eye(n).to('cuda')[perm[p].long()]).sum()
            newL = torch.einsum('ij,ij->i', A, torch.eye(n).to('cuda')[ci, :]).sum()
            #print(f"{iteration}/{p}: {newL - oldL}")
            progress = progress or newL > oldL + 1e-12
            iteration_loss = newL - oldL
            losses.append(iteration_loss)
            perm[p] = torch.Tensor(ci).to('cuda')
    ...
    return perm
# def find_permutation(ps: PermutationSpec, params_a, params_b, max_iter=100, init_perm=None):
#   """Find a permutation of `params_b` to make them match `params_a`."""
#   perm_sizes = {p: params_a[axes[0][0]].shape[axes[0][1]] for p, axes in ps.perm_to_axes.items()}
...
#   plt.title('Convergence of find_permutation')
