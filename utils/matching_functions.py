...
def is_doubly_stochastic(matrix, N):
    output_file="dosoc.csv"
    # Calculate row and column sums using PyTorch
    rows_sum = torch.sum(matrix, dim=1)
    cols_sum = torch.sum(matrix, dim=0)

    epsilon = 1e-3  # A small value to account for floating-point errors
    
    # Get the number of rows and columns
    num_rows, num_cols = matrix.size()
    rows_close_to_one = True
    cols_close_to_one = True

    # Check if each row is close to 1/N
    for i in range(num_rows):
        if not (torch.abs(rows_sum[i] - 1) < epsilon):
            print(f"Row {i} is not close to 1: {rows_sum[i]}")
            rows_close_to_one = False
            break
            


    # Check if each column is close to 1
    for j in range(num_cols):
        if not (torch.abs(cols_sum[j] - 1) < epsilon):
            #print(f"Column {j} is not close to 1: {cols_sum[j]}")
            print(f"Col {j} is not close to N: {cols_sum[j]}")
            cols_close_to_one = False
            break


    # Save row sums and column sums to a CSV file
    with open(output_file, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["Row Index", "Row Sum"])
        for i, sum_value in enumerate(rows_sum):
            writer.writerow([i, sum_value.item()])
        
        writer.writerow(["Column Index", "Column Sum"])
        for j, sum_value in enumerate(cols_sum):
            writer.writerow([j, sum_value.item()])


    return rows_close_to_one#, cols_close_to_one
...
def match_tensors_zipit(
    metric, corrsave_path, node, r=.5, a=0.3, b=.125, 
    print_merges=False, get_merge_value=False, check_doubly_stochastic=False, add_bias=False, 
    **kwargs
):
    """
    ...
    if check_doubly_stochastic:
        result = merge.T @ unmerge
        if not is_doubly_stochastic(result, num_models):
            print("The matrix is not doubly stochastic.")
...
def match_tensors_optimal(metric, corrsave_path, node, r=.5, add_bias=False, **kwargs):
    """ 
...
def match_tensors_permute(metric, corrsave_path, node, r=.5, get_merge_value=False, check_doubly_stochastic=False, add_bias=False, **kwargs):
    """
...
def match_tensors_kmeans(metric, corrsave_path, node, r=.5, get_merge_value=False, add_bias=False, **kwargs):
    """
...
def match_tensors_randperm(metric, corrsave_path, node, r=.5, get_merge_value=False, add_bias=False, **kwargs):
    """
