...
class ModelMerge(nn.Module):
    ...
    def compute_transformations(self, transform_fn, corrsave_path, reduce_ratio=.5, **kwargs):
        """
        ...
        for node in tqdm(nodes, desc="Computing transformations: "):
            if self.start_at is None or node >= self.start_at:
                ...
                if info['special_merge'] is not None:
                    ...
                    merge, unmerge = transform_fn(metric, corrsave_path, node, reduce_ratio, **kwargs)
            ...
            for key in keys:
                if key in merged_state_dict:
                    ...
                    if count_layer >= stop_layer:
                        new_value = self.graphs[0].model.state_dict()[key] * len(self.graphs)
                    else:
    ...
    def transform(self, model,
        ...
        self.merged_model.load_state_dict(self.get_merged_state_dict(interp_w), strict=False)
