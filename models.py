...
def BuildModel(model_name, img_dim, fc_nodes, nb_classes, myrank, ckp=None):

    loss = nn.CrossEntropyLoss()
    if model_name == "resnet20":
        model = resnet20(w = 8 , num_classes = nb_classes)
    elif model_name == "resnet50":
        model = torchvision.models.resnet50()
        num_ftrs = model.fc.in_features
        model.fc = nn.Linear(num_ftrs, nb_classes)
    elif model_name == "vgg16":
        model = vgg16(num_classes = nb_classes)
    elif model_name == "CNN":
        ...
        loss = nn.MSELoss()
    if ckp != None:
        model_path = os.path.join(ckp,"model_%d.pth"%myrank)
...
def LoadModel(img_dim, **kwargs):
    ...
    # Build model
    model, criterion = BuildModel(modelkey, ch_dim, fc_nodes, num_classes, myrank, ckp)
